...
namespace BattleMage.VR.Interaction
{
    ...
    public class InteractGrab : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            input.OnGrabPressed.AddListener((_arg) => { if (collidingObject != null) GrabObject(); });
            input.OnGrabReleased.AddListener((_arg) => { if (heldObject != null) ReleaseObject(); });
        }
        ...
        private void SetCollidingObject(Collider _other)
        {
            ...
            //We only want to handle the first thing we collide with
